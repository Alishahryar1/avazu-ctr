"""Exponential-to-Linear Cross Network for FCNv2."""
import torch
...
class Exponential2LinearCrossNetwork(nn.Module):
    """
    Exponential-to-Linear Cross Network (E2LCN) for FCNv2.
    
    Performs exponential feature crossing first (self-interaction),
    then linear feature crossing (with initial features).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Linear layers (interaction with initial features)
        for i in range(self.lin_num_layers):
            layer_idx = i + self.exp_num_layers
            x = self._cross_layer(x, x1, layer_idx)
