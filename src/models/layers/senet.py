...
class SENetLayer(nn.Module):
        ...
        if num_groups > 1:
            for i, dim in enumerate(self.feature_dims):
                assert dim % num_groups == 0, (
                    f"Dim {dim} not divisible by groups {num_groups}"
                )
    ...
    def forward(self, embeddings: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        if self.is_uniform:
            ...
            reshaped = flat_emb.view(
                batch_size, self.num_fields, self.num_groups, dim_per_group
            )
            ...
            for func in self.squeeze_funcs:
                ...
                if func == "mean":
                    ...
                    out.fill_(-1e9)
                    out.scatter_reduce_(
                        1, idx_expanded, flat_emb, reduce="amax", include_self=True
                    )
                elif func == "min":
                    out.fill_(1e9)
                    out.scatter_reduce_(
                        1, idx_expanded, flat_emb, reduce="amin", include_self=True
                    )
        ...
        if self.reweight_mode == "feature":
            if self.is_uniform:
                ...
                weights_expanded = (
                    weights.unsqueeze(2)
                    .expand(-1, -1, self.feature_dims[0])
        ...
        if self.use_layer_norm:
            if self.is_uniform:
                ...
                output = torch.cat(
                    [ln(t) for ln, t in zip(self.layer_norms, split_out)], dim=1
                )
