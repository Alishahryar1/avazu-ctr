...
def analyze_count_features_batched(
    lf: pl.LazyFrame, count_cols: list[str]
) -> dict[str, dict]:
    """
    ...
    for col in count_cols:
        ...
        print(
            f"    P25: {s['p25']:,.0f}, P50: {s['p50']:,.0f}, P75: {s['p75']:,.0f}, P90: {s['p90']:,.0f}"
        )
...
def analyze_cumcount_features(
    lf: pl.LazyFrame, cumcount_cols: list[str]
) -> dict[str, dict]:
    """
...
def analyze_time_delta(lf: pl.LazyFrame) -> dict:
    ...
    lf_with_delta = (
        ...
        .with_columns(
            [
                (pl.col("_abs_hour") - pl.col("_abs_hour").shift(1).over("user_proxy"))
                .fill_null(0)
                .clip(0, None)
...
def analyze_hourly_impressions(lf: pl.LazyFrame) -> dict:
    ...
    hourly_counts = (
        lf_with_proxy.group_by(["user_proxy", "hour"])
        .len()
...
def analyze_cardinalities(
    lf: pl.LazyFrame, cat_cols: list[str], min_freq: int
) -> dict[str, dict]:
    """
    ...
    for col, info in sorted_results.items():
        print(
            f"{col:<30} {info['cardinality']:>12,} {info['total_unique']:>15,} {info['filtered_out']:>10,}"
        )
...
def main():
    ...
    lf_with_features = (
        lf.with_columns(get_user_proxy_expression())
    ...
    results["cardinalities"] = analyze_cardinalities(
        lf_with_features, all_cat_cols, CONFIG["min_freq"]
    )
    gc.collect()

    # 2. Count features
    results["count_stats"] = analyze_count_features_batched(
        lf_with_features, COUNT_FEATURE_COLS
    )
    gc.collect()

    # 3. Cumulative count features
    results["cumcount_stats"] = analyze_cumcount_features(
        lf_with_features, CUMCOUNT_COLS
    )
    gc.collect()
