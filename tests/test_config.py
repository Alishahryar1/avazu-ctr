...
class TestConfig(unittest.TestCase):
    ...
    def test_required_keys_exist(self):
        ...
        if "models" in model_config:
            ...
            self.assertGreater(
                len(model_config["models"]), 0, "Ensemble must have at least one model"
            )
            ...
            self.assertGreater(
                len(model_config["heads"]), 0, "Must have at least one head"
            )
        else:
            # Single model config (GatedDCN or STEC)
            # Should have either 'use_dcn' (GatedDCN) or 'stec_num_layers' (STEC)
            has_model_keys = (
                "use_dcn" in model_config or "stec_num_layers" in model_config
            )
    ...
    def test_config_value_ranges(self):
        ...
        if dense_lr is not None:
            self.assertGreater(
                float(dense_lr), 0, "dense optimizer lr must be positive"
            )
        ...
        def check_gated_dcn_config(cfg: dict[str, Any]) -> None:
            ...
            if "use_senet" in cfg and "use_feature_gating" in cfg:
                if cfg["use_senet"] and cfg["use_feature_gating"]:
                    self.fail(
                        "Config has both use_senet and use_feature_gating enabled"
                    )
