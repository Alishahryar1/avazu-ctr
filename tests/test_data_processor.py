        ...
        test_data = pl.DataFrame(
            {
                "hour": [
                    ...
                    "14102223",
                    "14110105",
                ]  # 2014-10-21 00:00, 2014-10-22 23:00, 2014-11-01 05:00
            }
        )
...
class TestDataProcessorVocabulary(unittest.TestCase):
    ...
    def test_build_vocabularies_mapping_starts_at_one(self):
        ...
        test_data = pl.DataFrame({"cat": ["x", "y", "z"] * 5})

        _, feat_maps = build_vocabularies(test_data.lazy(), ["cat"], min_freq=1)
...
class TestUserProxyFeature(unittest.TestCase):
    ...
    def test_user_proxy_with_special_characters(self):
        ...
        test_data = pl.DataFrame(
            {"device_ip": ["192.168.1.1"], "device_model": ["iPhone-12_Pro Max"]}
        )
...
class TestCountFeatures(unittest.TestCase):
    ...
    def test_compute_count_features_no_data_leakage(self):
        ...
        # Train has ip1 only
        train_data = pl.DataFrame({"device_ip": ["ip1", "ip1"]}).lazy()

        # Test has ip2 only (not in train)
        test_data = pl.DataFrame({"device_ip": ["ip2", "ip2", "ip2"]}).lazy()
    ...
    def test_count_features_dtype(self):
        ...
        train_data = pl.DataFrame({"device_ip": ["ip1", "ip1", "ip2"]}).lazy()

        test_data = pl.DataFrame({"device_ip": ["ip1"]}).lazy()
...
class TestCumulativeCountFeatures(unittest.TestCase):
    ...
    def test_cumulative_count_multiple_columns(self):
        ...
        test_data = pl.DataFrame(
            {"device_ip": ["ip1", "ip1", "ip2"], "user_proxy": ["u1", "u2", "u1"]}
        )
...
class TestHourlyAggregatedFeatures(unittest.TestCase):
    ...
    def test_compute_hourly_features_basic(self):
        ...
        self.assertEqual(
            test_impressions[2], 1
        )  # u3@14102100 (not in train, defaults to 1)
    ...
    def test_hourly_features_no_leakage(self):
        ...
        train_data = pl.DataFrame(
            {"user_proxy": ["u1", "u1"], "hour": ["14102100", "14102100"]}
        ).lazy()
        ...
        self.assertTrue(
            all(c == 1 for c in test_result["user_hourly_impressions"].to_list())
        )
...
class TestTimeDeltaFeatures(unittest.TestCase):
    ...
    def test_compute_time_delta_basic(self):
        ...
        result = compute_time_delta_features(
            test_data, group_col="user_proxy"
        ).collect()
        ...
        test_data = pl.DataFrame(
            {
                ...
                "hour": [
                    "14102100",
                    "14102200",
                ],  # Oct 21 00:00 -> Oct 22 00:00 = 24 hours
            }
        ).lazy()
...
class TestPreviousClickCount(unittest.TestCase):
    ...
    def test_compute_prev_click_count_basic(self):
        ...
        test_data = pl.DataFrame({"user_proxy": ["u1", "u1", "u1", "u1"]}).lazy()

        result = compute_previous_click_count(
            test_data, group_col="user_proxy"
        ).collect()
    ...
    def test_compute_prev_click_count_multiple_users(self):
        ...
        test_data = pl.DataFrame({"user_proxy": ["u1", "u2", "u1", "u2", "u1"]}).lazy()
    ...
    def test_prev_click_count_all_unique(self):
        ...
        test_data = pl.DataFrame({"user_proxy": ["u1", "u2", "u3", "u4"]}).lazy()
